...
const ContentMédico = () => {
  ...
  const areaDeAtuacao = secureLocalStorage.getItem('AreadeAtuacao') || ''
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.post(`${config.apiBaseUrl}/api/get-casos-clinicos/${id}`)
        setSelectedCasosClinicos(response.data.HistoricoCasosClinicos)
      } catch (error) {
  ...
  return (
    <>
      <div className='flex justify-center mt-10 gap-5 mb-3'>
        <h1 className="font-bold text-blue-950 text-2xl text-center sm:text-xl"> Casos Clinicos para {areaDeAtuacao}</h1>
      </div>

      <h1 className="font-bold text-blue-600 text-2xl text-center sm:text-xl">Quantidade de Casos Clinicos: {selectedCasosClinicos.length}</h1>
      ...
      <div className="flex justify-center items-center flex-wrap gap-5">
        {!CreateRequestMutation.isSuccess && !CreateRequestMutation.isLoading && selectedCasosClinicos.length === 0 && (
        ...
        {CreateRequestMutation.isSuccess && (
          <div className="flex gap-7 flex-wrap mb-7 justify-center items-center sm:gap-10">
            {selectedCasosClinicos.map((casosClinicosData, index) => (
              <div
                key={index}
        ...
        {notHorarios && <PopUpMedicoHoras onClose={() => setNotHorarios(false)} />}
      </div>

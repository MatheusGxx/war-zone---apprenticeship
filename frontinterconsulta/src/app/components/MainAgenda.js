...
export const MainAgenda = () => {
  ...
  const UpdateConsulta = useMutation(async (valueRequest) => {
    const response = await axios.post('http://localhost:8080/api/update-consulta', valueRequest)
    return response.data
  },
  {
    onSettled: () => {  
  ...
  const DeleteCasoClinicoPacienteParticular = useMutation(
    async (body) => {
      try {
        const response = await axios.delete(`http://localhost:8080/api/delete-caso-clinico-paciente-particular`,{ data: body })
        return response.data; 
      } catch (error) {
  ...
  const getLaudo = useMutation(async (valueRequest) => {
    const response = await axios.post('http://localhost:8080/api/get-laudo', valueRequest, {
      responseType: 'json',
    });
  ...
  const RequestGetConsultas = async () => {
    try{
      const request = await axios.get(`http://localhost:8080/api/get-consultas/${id}`)
      return request.data
    }catch(e){
  ...
  const RequestGetConsultasConfirmadas = async (id) =>{
    const response = await axios.get(`http://localhost:8080/api/get-consultas-links-doctor/${id}`)
              ...
              >
                {consultasConfirmadasComLinkPreenchido.map((consultas, key) => (
                  <MenuItem key={key} value={consultas._id}>
                   ...
                   <Image src={`http://localhost:8080/${consultas.FotoPaciente}`} alt="Foto do Médico" width={50} height={50} className="rounded-full" />
